...
def test_for_gen_z(B_max, D, H):
    X = np.array([0.8, 1]) * np.sum(D, axis=0)
    X = X.astype(int)
    for x in X:
        S_T = x
        Beta_max = D / S_T
        S = Beta_max / (H * B_max)
        # print("The original S is {}.".format(S))
...
def test_for_amax(B_max, Beta_max, H, S):
    am, max_index = amax(Beta_max, S)
    i_sum_I, i_L_I, i_L_II = divide_set(am, B_max, Beta_max, H)
    ii_sum_I, ii_L_I, ii_L_II = divide_set(am * 0.999, B_max, Beta_max, H)
    print('amax: {}, index: {}.'.format(am, max_index))
    print('The s-max in S is {}.'.format((S[max_index])))
    print('The sum of Beta is {}, when a is max.'.format(i_sum_I))
    print('The sum of Beta is {}, when a is less max.'.format(ii_sum_I))
...
def figure_for_Ea(B_max, D, H, S_T, delta):
    print('result for test.')
    logger.info("The drawing for Ea starts!")
    Beta_max = D / S_T
    S = Beta_max / (H * B_max)
    xmax, max_index = amax(Beta_max, S)
    x = np.arange(0, xmax, 1e-7).tolist()
    y = []
    for m in x:
        ym = Ea(H, B_max, Beta_max, delta, m)[0]
        y.append(ym)
    plt.plot(x, y, label="E(a)")
    plt.legend()
    plt.savefig("E-a.pdf")
    logger.info("The drawing for Ea ends!")


def figure_for_node_amount(B_max, D, H, S_T, delta):
    print('result for main 1.')
    logger.info("We start drawing figure for data requirements!")
    ...
    for x in X:
        ...
        H_n = H[0: N]
        Beta_max = D_n / S_T
        S = Beta_max / (H_n * B_n_max)
        bmax = min(B_n_max)

        Yo.append(min_Ea(H_n, B_max, Beta_max, S, delta)[0])
        print("The amount of nodes is {}.".format(N))
        Y_cop.append(cop(bmax, H_n, D_n, delta)[0])
        Y_tpc.append(TPC(bmax, H, delta)[2])

    print(Yo)
    plt.figure(figsize=(10, 10))
    plt.plot(X, Yo, "sr-", label='the proposed', linewidth=5, markersize=20)
    plt.plot(X, Y_cop, "og-", label='cop method', linewidth=5, markersize=20)
    plt.plot(X, Y_tpc, "pb-", label='tpc method', linewidth=5, markersize=20)
    plt.xlabel('Number of nodes, $N$', fontsize='24')
    ...
    plt.savefig("FL for node numbers.pdf", bbox_inches='tight')
    logger.info("We end drawing figure for data requirements!")


def figure_for_channel_gain(B_max, D, S_T, delta):
    print('result for main 2.')
    ...
    Beta_max = D / S_T
    for x in X:
        # H = np.random.normal(x, 5, 15)
        H = np.array([x] * 20)
        S = Beta_max / (H * B_max)

        bmax = min(B_max)

        Yo.append(min_Ea(H, B_max, Beta_max, S, delta)[0])
        Y_tpc.append(TPC(bmax, H, delta)[2])
        Y_cop.append(cop(bmax, H, D, delta)[0])
    ...
    plt.plot(X, Y_cop, "og-", label='cop method', linewidth=5, markersize=20)
    plt.plot(X, Y_tpc, "pb-", label='tpc method', linewidth=5, markersize=20)
    plt.xlabel('The mean of channel gain, $h$', fontsize='24')
    plt.ylabel('MSE', fontsize='24')
    plt.gca().ticklabel_format(style='scientific', scilimits=(-2, 2), useMathText=True)
    plt.legend(loc='upper right', fontsize='24')
    plt.gca().yaxis.get_offset_text().set_fontsize(20)
    plt.grid()
    plt.tick_params(labelsize=24)
    plt.savefig("FL for channel gain.pdf", bbox_inches='tight')


def figure_for_noise_power(B_max, D, H, S_T):
    print('result for main 3.')
    Beta_max = D / S_T
    S = Beta_max / (H * B_max)
    Yo = []
    Y_cop = []
    ...
    for x in X:
        ...
        Yo.append(min_Ea(H, B_max, Beta_max, S, delta)[0])
        print("The noise power is {}.".format(delta))
        Y_cop.append(cop(bmax, H, D, delta)[0])
        Y_tpc.append(TPC(bmax, H, delta)[2])
    ...
    plt.plot(X, Y_cop, "og-", label='cop method', linewidth=5, markersize=20)
    plt.plot(X, Y_tpc, "pb-", label='tpc method', linewidth=5, markersize=20)
    plt.xlabel('The power of noise, $\delta$', fontsize='24')
    plt.ylabel('MSE', fontsize='24')
    plt.gca().ticklabel_format(style='scientific', scilimits=(-2, 2), useMathText=True)
    plt.legend(loc='upper right', fontsize='24')
    plt.gca().yaxis.get_offset_text().set_fontsize(20)
    plt.grid()
    plt.tick_params(labelsize=24)
    plt.savefig("FL for noise power.pdf", bbox_inches='tight')


def figure_for_data_require(B_max, D, H, delta):
    print('result for main 4.')
    Yo = []
    Ao = []
    Y_cop = []
    A_cop = []
    Y_tpc = []
    X = np.array([0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]) * np.sum(D, axis=0)
    # X = np.array([1]) * np.sum(D, axis=0)
    ...
    for x in X:
        S_T = x
        Beta_max = D / S_T
        # print("Beta_max is {}.".format(Beta_max))
        S = Beta_max / (H * B_max)
        bmax = min(B_max)

        yo, ao = min_Ea(H, B_max, Beta_max, S, delta)
        Yo.append(yo)
        Ao.append(ao)
        y_cop, a_cop, b_cop = cop(bmax, H, D, delta)
        Y_cop.append(y_cop)
        A_cop.append(a_cop)
        Y_tpc.append(TPC(bmax, H, delta)[2])
        print('S_T is {}: FL is {}, ao is {}; COP is {}, a_cop is {}.'.format(x, yo, ao, y_cop, a_cop))
        # print('B_cop is {}.'.format(b_cop))
        # print('The max of B_cop is {} and bmax is {}.'.format(max(b_cop), bmax))
        for b in b_cop:
            if b > bmax:
                print("There is a b:{} which breaks the b constraints.".format(b))
            else:
                continue
    ...
    plt.plot(X, Y_cop, "og-", label='cop method', linewidth=5, markersize=20)
    plt.plot(X, Y_tpc, "pb-", label='tpc method', linewidth=5, markersize=20)
    plt.xlabel('The data requirement, $S_T$ bits', fontsize='24')
    plt.ylabel('MSE', fontsize='24')
    plt.gca().ticklabel_format(style='scientific', scilimits=(-2, 2), useMathText=True)
    plt.legend(loc='upper right', fontsize='24')
    plt.gca().yaxis.get_offset_text().set_fontsize(20)
    plt.grid()
    plt.tick_params(labelsize=24)
    plt.savefig("FL for data requirement.pdf", bbox_inches='tight')

# # B_max for general.
# B_max = np.random.normal(0.01, 0.002, 20)
# # print('B_max:')
# # print(B_max)
# # B_max for data requirement.
# # B_max = np.array([mp.mpmathify(FRa(1, 100))] * 20)
# # B_max = np.array([0.01] * 20)
# logger.info("B_max is {}.".format(B_max))
#
# # OH = np.rint(np.random.normal(50, 5, 20))
# # H = []
# # for oh in OH:
# #     H.append(mp.mpmathify(oh))
# # H = np.array(H)
# H = np.rint(np.random.normal(50, 5, 20))
# logger.info('H is {}.'.format(H))
#
# D = np.random.binomial(n=6000, p=0.8, size=20)
# delta = 0.1
# # S_T = np.sum(D, axis=0)
# S_T = np.sum(D[0:10], axis=0)
# # while np.sum(D, axis=0) < S_T or min(Beta_max) < 1/len(H):
# logger.info('D is {}.'.format(D))
# # Beta_max = []
# # for i in range(len(D)):
# #     Beta_max.append(mp.mpmathify(FRa(D[i], S_T)))
# # Beta_max = np.array(Beta_max)
# # Beta_max = D / S_T
# # print("Beta_max:{}.".format(Beta_max))
#
# # S = Beta_max / (H * B_max)
# # test_for_amax(B_max, Beta_max, H, S)
# # print("S:")
# # print(S)
# # a_max, max_index = amax(Beta_max, S)
# # print('amax: {}'.format(xmax))
# # r = Fa(Beta_max, H, B_max, xmax*0.99)
# # print('r: {}'.format(r))
# figure_for_data_require(B_max, D, H, delta)
# # figure_for_Ea(B_max, D, H, S_T, delta)
# # figure_for_node_amount(B_max, D, H, S_T, delta)
# # figure_for_channel_gain(B_max, D, S_T, delta)
# # figure_for_noise_power(B_max, D, H, S_T)
# # test_for_gen_z(B_max, D, H)
# # test_for_Fa_min(Beta_max, H, B_max, a_max)
# # test_for_min_sci_Hk_a(Beta_max, H, B_max)
